...
void reverse(VALUE_TYPE* array, int len) {
...
void my_get_x_b(int m, int n, const int * csrRowPtrA, const int *csrColIdxA, const VALUE_TYPE *csrValA, VALUE_TYPE **x_add, VALUE_TYPE **b_add)
{
    VALUE_TYPE *x_ref = (VALUE_TYPE *)malloc(sizeof(VALUE_TYPE) * n);
    int i,j;
    for ( i = 0; i < n; i++)
    ...
    for (i = 0; i < m; i++)
    {
        y[i] = 0;
        for (j = csrRowPtrA[i]; j < csrRowPtrA[i+1]; j++)
            y[i] += csrValA[j] * x_ref[csrColIdxA[j]];
    }
